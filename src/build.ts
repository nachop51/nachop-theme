...
import nightySchema from './schemas/nighty.js'

function filePath (name: string) {
  return path.join(cwd(), 'themes', `${name.toLowerCase().replaceAll(' ', '-')}.json`)
}

const themes: Array<{ name: string }> = [
  ...
  buildThemeFromColors({ colors: piggySchema, name: 'Piggy', isBordered: true }) as { name: string },
  buildThemeFromColors({ colors: piggySchema, name: 'Piggy', isBordered: false }) as { name: string },
  buildThemeFromColors({ colors: nightySchema, name: 'Nighty', isBordered: true }) as { name: string }
]
